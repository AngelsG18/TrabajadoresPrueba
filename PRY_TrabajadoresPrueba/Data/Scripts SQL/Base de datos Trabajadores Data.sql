...
CREATE TABLE [TipoDocumento](
	...
	[DescDocumento] VARCHAR(50) NOT NULL
)

GO

INSERT INTO [TipoDocumento] ([CodDocumento], [DescDocumento])
...
CREATE PROCEDURE [Sp_Busca_trabajadores]
@OPCION AS VARCHAR(1)
AS
SET NOCOUNT ON
BEGIN
...
SELECT 
	A.Id
	,TipoDocumento = (SELECT TOP 1 [DescDocumento] FROM [TipoDocumento] B WHERE B.CodDocumento = A.TipoDocumento)
	,A.NumeroDocumento
	,A.Nombres
	,A.Sexo
	,B.Id as IdDepa
	,B.[NombreDepartamento] as Departamento
	,C.Id as IdProv
	,C.[NombreProvincia] as Provincia
	,D.Id as IdDist
	,D.[NombreDistrito] as Distrito
FROM #TRABAJADORES A
INNER JOIN Departamento B ON A.IdDepartamento = B.Id
INNER JOIN Provincia C ON A.IdProvincia = C.Id
INNER JOIN Distrito D ON A.IdDistrito = D.Id

END

GO

CREATE PROCEDURE [Sp_Mostrar_Trabajadores]
AS
SET NOCOUNT ON
BEGIN
SELECT 
...
CREATE PROCEDURE [Sp_Buscar_Trabajador_Id]
@ID AS INT
AS
SET NOCOUNT ON
BEGIN
...
CREATE PROCEDURE [Sp_Man_Insertar_Trabajador]
@TIP_DOCUMENTO AS VARCHAR(3),
@NUM_DOCUMENTO AS VARCHAR(50),
@NOMBRES AS VARCHAR(500),
@SEXO AS VARCHAR(1),
...
BEGIN
	BEGIN TRANSACTION
		...
		[Nombres], [Sexo], [IdDepartamento], 
		[IdProvincia], [IdDistrito])
		VALUES 
		(@TIP_DOCUMENTO, @NUM_DOCUMENTO,
		@NOMBRES, @SEXO, @ID_DEPARTAMENTO,
		@ID_PROVINCIA, @ID_DISTRITO)
...
CREATE PROCEDURE [Sp_Man_Actualizar_Trabajador]
@ID AS INT,
@TIP_DOCUMENTO AS VARCHAR(3),
@NUM_DOCUMENTO AS VARCHAR(50),
@NOMBRES AS VARCHAR(500),
...
BEGIN
	BEGIN TRANSACTION
		...
		Update Trabajadores
		set [TipoDocumento] = @TIP_DOCUMENTO,
...
CREATE PROCEDURE [Sp_Man_Eliminar_Trabajador]
@ID AS INT
AS
SET NOCOUNT ON
SET XACT_ABORT ON
